    ...
    <main>
      ...
      <div class="schedule-container">
          <div class="schedule-content">
            <div class="schedule">
              <h2 class="topic">FUNOLYMPIC MATCH SCHEDULE</h2>
              <div class="schedule-image">
                <img src="{{ asset('./images/gallery-img-1.jpg') }}" alt="News Image" />
              </div>
            </div>
            <section class="schedule-card">
              @foreach($schedules as $schedule)
                <div class="card">
                    <h3 class="group">{{ $schedule->game->name }}</h3>
                    <div class="card-content">
                        <div class="card-images">
                            <img src="{{ (!empty($schedule->teamA->country_icon)) ? url('upload/admin_images/'.$schedule->teamA->country_icon) : url('upload/no_image.jpg') }}" alt="{{ $schedule->teamA->country_name }}">
                            <p>{{ $schedule->teamA->country_name }}</p>
                        </div>
                        <div class="image-vs">
                            <h3 class="vs">VS</h3>
                        </div>
                        <div class="card-image">
                        <img src="{{ (!empty($schedule->teamB->country_icon)) ? url('upload/admin_images/'.$schedule->teamB->country_icon) : url('upload/no_image.jpg') }}" alt="{{ $schedule->teamB->country_name }}">
                            <p>{{ $schedule->teamB->country_name }}</p>
                        </div>
                    </div>
                    <h3 class="date">{{ \Carbon\Carbon::parse($schedule->date)->format('F d, Y') }}</h3>
                    <h3 class="time">{{ \Carbon\Carbon::parse($schedule->time)->format('h:i A') }}</h3>
                </div>
              @endforeach
            </section>
          </div>
